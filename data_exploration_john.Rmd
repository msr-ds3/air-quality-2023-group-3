...
country_locs <- read_csv("countries_codes_and_coordinates.csv") %>% 
  rename(lat="Latitude (average)", long="Longitude (average)")
...
lm <- lm(pm25 ~ lat*month +long*month + is_weekend + countryCode,data=country_data_17_19)

summary(lm)
predictions <- country_data_2020 %>% 
  add_predictions(lm)

1-mean((predictions$pm25 -predictions$pred)^2)/var(predictions$pm25)
sqrt(mean((predictions$pm25 -predictions$pred)^2))

predictions %>% 
  group_by(countryCode) %>% 
  summarize(pred_mean = mean(pred), actual_mean = mean(pm25)) %>% 
  mutate(abs_diff=pred_mean-actual_mean, rel_change=(actual_mean-pred_mean)/actual_mean * 100) %>% 
  filter(rel_change > -150) %>% 
  ggplot(aes(x=countryCode, y=rel_change, color=rel_change)) +
  geom_point() +
